...
async function updateLeaderboards(callerUsername){
  try{
    ...
    const tttSorted = [...users].sort((a, b) => b.tttWin - a.tttWin)
    let tttText = "\nTTT Win Leaderboard\n"
    
    const top10TTT = tttSorted.slice(0, 10)
    top10TTT.forEach((user, index) => {
      tttText += `${index + 1}. ${user.username}: *${user.tttWin}*\n`
    })

    const callerTTTRank = tttSorted.findIndex(u => u.username === callerUsername)
    if(callerTTTRank >= 10){
      const caller = tttSorted[callerTTTRank]
      tttText += `...\n${callerTTTRank + 1}. ${caller.username}: ${caller.tttWin}\n`
    }
    ...
    const fullLeaderboardText = tokenText + tttText + timeText
