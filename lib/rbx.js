...
async function getRobloxAvatar(userId, apiKey){
    try{
        ...
        const folderName = `avatar_${userId}`
        ...
        const zipName = `avatar_${userId}.zip`
...
async function download(username, KEY){
  ...
  let filePath = await getRobloxAvatar(userID, KEY)
  info[1] = filePath
  info[0] = log
  return info
}
